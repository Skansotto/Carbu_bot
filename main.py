        ...
        for messaggio in updates:
            ...
            if "location" in messaggio["message"].keys():
                ...
                if (len(risultato) == 1):
                    ...
                    queryRicerca = f"SELECT *, ( 6371 * acos( cos(radians({latitudine})) * cos(radians(benzinai.latitudine)) * cos(radians(benzinai.longitudine) - radians({longitudine})) + sin(radians({latitudine})) * sin(radians(benzinai.latitudine)) ) ) as distanza FROM benzinai JOIN prezzi ON benzinai.idImpianto = prezzi.idImpianto WHERE ( 6371 * acos( cos(radians({latitudine})) * cos(radians(benzinai.latitudine)) * cos(radians(benzinai.longitudine) - radians({longitudine})) + sin(radians({latitudine})) * sin(radians(benzinai.latitudine)) ) ) <={maxKm} AND descCarburante LIKE '%{tipoCarburante}%' ORDER BY prezzo asc LIMIT 3;"
